...
adminScreen('a_pays', async (ctx) => {
  ...
  const k = kb();
  for (const p of data) k.text(`${methodLabel(ctx.lang, p.method)} ${t(ctx, 'admin.payRow', { amount: money(p.amount_usd), name: esc(p.users.first_name || p.users.tg_id) })}`, to('a_pay', p.order_id)).row();
  k.text(t(ctx, 'btn.back'), to('admin'));
...
adminScreen('a_pay', async (ctx, [orderId]) => {
  ...
  return { text: t(ctx, 'admin.payTitle', { rule: RULE, name: esc(row.users.first_name || ''), tg: row.users.tg_id, amount: money(row.amount_usd), tx: esc(row.external_id || '—'), date: fmtDate(row.created_at, ctx.lang) }),
    kb: kb().add({ text: t(ctx, 'admin.payBtn.approve', { amount: money(row.amount_usd) }), data: to('a_payok', orderId), style: 'success' }).row()
